...
for (k in 1:n_resample) {
  ...
  for (krpm in 1:nrow(params_random)) {
    ...
    d_res <- rbind(d_res, data.frame(krpm = krpm, k = k, ntrees = md$best_iter, runtm = runtm, auc = auc))
  }
  
}

d_res_avg <- d_res %>% group_by(krpm) %>% summarize(ntrees = mean(ntrees), runtm = mean(runtm), auc = mean(auc))
